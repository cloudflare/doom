...
// Doom-wasm: per-process identifier used as the routing handle for the
// WebSocket-based net module. The dedicated server is hard-coded to id=1
// (see net_websockets.c::ResolveAddress); each client picks a random id
// in [0, 0xFFFE).
...
static void D_Disconnected(void)
{
    ...
    // disconnected from server (doom-wasm stdout protocol code 9)
...
boolean D_InitNetGame(net_connect_data_t *connect_data)
{
    ...
    {
        // Doom-wasm: the server has instanceUID=1 (the well-known ID
        // used by clients to address the server in the WebSocket
        // protocol).
        instanceUID = 1;
    ...
    else
    {
        ...
        // address. (Under doom-wasm this is ignored: the websocket
        // module always routes to instanceUID=1.)
        //
        ...
        if (i > 0)
        {
            ...
            // The server is hard-coded to instanceUID=1 in the
            // websocket protocol.
            addr = net_websockets_module.ResolveAddress("1");
            NET_ReferenceAddress(addr);
