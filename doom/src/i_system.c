...
#ifdef __EMSCRIPTEN__
    // Stop the emscripten main loop and force the runtime to exit so the
    // browser tab actually shuts down (a plain exit() would otherwise be
    // intercepted by emscripten and leave the main loop running).
