...
#ifdef __EMSCRIPTEN__
    // Doom-wasm: schedule the per-frame callback with the browser before
    // I_InitGraphics() runs, otherwise emscripten warns:
    //   "emscripten_set_main_loop_timing: Cannot set timing mode for main
    //    loop since a main loop does not exist!"
