...
{ 
...
#ifdef __EMSCRIPTEN__
    // Doom-wasm: when key_fullscreen ('f' by default) is held, request the
    // browser to put the canvas into fullscreen.
    if (gamekeydown[key_fullscreen])
    {
        ...
        if (!fsce.isFullscreen)
        {
            ...
            // Yield briefly so the browser can transition before we
            // continue building ticcmds.
